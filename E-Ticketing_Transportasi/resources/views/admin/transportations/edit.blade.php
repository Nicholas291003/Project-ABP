...
@section('title', 'Edit Transportasi - Travelgo')
...
<div class="p-8 space-y-8 flex-1 max-w-5xl">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-3xl font-black text-white tracking-tight">Edit Data Transportasi</h1>
            <p class="text-sm text-slate-400 mt-1">Lakukan pembaruan atau perubahan data spesifikasi armada kendaraan.</p>
        </div>
    ...
    <div class="bg-slate-950/40 backdrop-blur-md border border-slate-800/80 rounded-2xl p-6 md:p-8 shadow-xl shadow-black/10">
        <form action="{{ route('admin.transportations.update', $transportation->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            
            <h3 class="text-lg font-black text-slate-200 border-b pb-3 border-slate-800/60 flex items-center">
                <i data-lucide="edit-3" class="w-5 h-5 text-teal-400 mr-2"></i> Informasi Kendaraan
            </h3>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="jenis" id="select_jenis" required class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-sm text-slate-200 focus:outline-none focus:border-teal-400 transition-all">
                        ...
                        <option value="kereta" {{ old('jenis', $transportation->jenis) == 'kereta' ? 'selected' : '' }}>Kereta Api</option>
                        <option value="bus" {{ old('jenis', $transportation->jenis) == 'bus' ? 'selected' : '' }}>Bus / Travel</option>
                        <option value="pesawat" {{ old('jenis', $transportation->jenis) == 'pesawat' ? 'selected' : '' }}>Pesawat</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <div class="flex rounded-xl overflow-hidden shadow-sm">
                        @php
                            $kodeParts = explode('-', $transportation->kode);
                            $prefix = $kodeParts[0] ?? 'BUS';
                            $suffix = $kodeParts[1] ?? '';
                        @endphp
                        <select name="kode_prefix" id="kode_prefix" required class="bg-slate-800 border border-slate-800 border-r-0 text-slate-200 text-sm font-bold px-4 py-2.5 focus:outline-none">
                            <option value="TRN" {{ old('kode_prefix', $prefix) == 'TRN' ? 'selected' : '' }}>TRN</option>
                            <option value="BUS" {{ old('kode_prefix', $prefix) == 'BUS' ? 'selected' : '' }}>BUS</option>
                            <option value="FLT" {{ old('kode_prefix', $prefix) == 'FLT' ? 'selected' : '' }}>FLT</option>
                        </select>
                        <input type="text" name="kode_suffix" required placeholder="Contoh: 002" value="{{ old('kode_suffix', $suffix) }}" class="w-full bg-slate-950 border border-slate-800 rounded-r-xl px-4 py-2.5 text-sm text-slate-200 focus:outline-none focus:border-teal-400 uppercase transition-all">
                    </div>
                    @error('kode')
                ...
                <div>
                    ...
                    <input type="text" name="nama" value="{{ old('nama', $transportation->nama) }}" required placeholder="Contoh: Sinar Jaya Raya" class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-sm text-slate-200 focus:outline-none focus:border-teal-400 transition-all">
                </div>
                ...
                <div>
                    ...
                    <select name="kelas" required class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-sm text-slate-200 focus:outline-none focus:border-teal-400 transition-all">
                        <option value="" disabled>Pilih Kelas Layanan</option>
                        <option value="Economy" {{ old('kelas', $transportation->kelas) == 'Economy' ? 'selected' : '' }}>Economy</option>
                        <option value="Eksekutif" {{ old('kelas', $transportation->kelas) == 'Eksekutif' ? 'selected' : '' }}>Eksekutif</option>
                        <option value="Luxury" {{ old('kelas', $transportation->kelas) == 'Luxury' ? 'selected' : '' }}>Luxury</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="jumlah_kursi" value="{{ old('jumlah_kursi', $transportation->jumlah_kursi) }}" required min="1" placeholder="0" class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-sm text-slate-200 focus:outline-none focus:border-teal-400 transition-all">
                </div>
                ...
                <div>
                    ...
                    <select name="status" required class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-sm text-slate-200 focus:outline-none focus:border-teal-400 transition-all">
                        <option value="aktif" {{ old('status', $transportation->status) == 'aktif' ? 'selected' : '' }}>Aktif</option>
                        <option value="maintenance" {{ old('status', $transportation->status) == 'maintenance' ? 'selected' : '' }}>Dalam Perbaikan (Maintenance)</option>
                        <option value="nonaktif" {{ old('status', $transportation->status) == 'nonaktif' ? 'selected' : '' }}>Nonaktif</option>
                    </select>
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="fasilitas" rows="3" placeholder="Contoh: AC, Reclining Seat, Snack, Toilet..." class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-sm text-slate-200 focus:outline-none focus:border-teal-400 transition-all">{{ old('fasilitas', $transportation->fasilitas) }}</textarea>
            </div>

            <div class="flex items-center justify-end space-x-3 pt-4 border-t border-slate-800/60">
                <a href="{{ route('admin.transportations.index') }}" class="px-5 py-2.5 rounded-xl border border-slate-700 bg-rose-500/10 text-rose-400 text-sm font-semibold hover:bg-rose-500/20 text-center transition-all">
                    Batal
                </a>
